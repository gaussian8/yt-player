...
export default {
  ...
  methods: {
    initialize: function () {
      axios.get('/yt/api/entities')
      .then(res => {
        this.entities = res.data
        this.entity = res.data[0].value
        axios.get('/yt/api/entities/' + this.entity)
        .then(res => {
          this.play_list_count = res.data.play_list_count
          this.played_list_count = res.data.played_list_count
        })
      })
      axios.get('/yt/api/playlists/' + this.$route.params.id)
      .then(res => {
        this.items = res.data
      })
    },
    play: function (play_list) {
      axios.post('/yt/api/entities/' + this.entity + '/play', play_list)
      .then(res => {
        this.play_list_count = res.data.play_list_count
        this.played_list_count = res.data.played_list_count
      })
    ...
    play_pause: function () {
      axios.post('/yt/api/entities/' + this.entity + '/play_pause')
    },
    prev: function () {
      this.loading.prev = true
      axios.post('/yt/api/entities/' + this.entity + '/prev')
      .then(res => {
        this.play_list_count = res.data.play_list_count
        this.played_list_count = res.data.played_list_count
    ...
    next: function () {
      ...
      axios.post('/yt/api/entities/' + this.entity + '/next')
      .then(res => {
        this.play_list_count = res.data.play_list_count
        this.played_list_count = res.data.played_list_count
